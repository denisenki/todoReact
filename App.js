...
import {AddButton, List, Tasks} from './components';
...
function App() {
  ...
  const [list, setLists] = React.useState(
    DB.lists.map(ithem => {
    ithem.color = DB.colors.filter(
      color => color.id === ithem.colorId)[0].name;
      return ithem;
  }));
  ...
  return (
    <div className="todo">
      <div className="todo__sidebar">
        <List items={[{
          id: 1, icon: (<svg width="14" height="12" viewBox="0 0 14 12" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          ), name: 'Все задачи', active: false, isRemovable: false
        }]} />
        <List items={list} isRemovable={true} onRemove={onRemove}/>
        <AddButton color={DB.colors} onAdd={onAdd} isRemovable={false}/>
      </div>
      <Tasks/>
